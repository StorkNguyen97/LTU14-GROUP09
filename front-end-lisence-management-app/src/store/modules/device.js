...
const actions = {
  getList: context => {
    return axios.get(DEVICE_API).then(response => {
      context.commit("GET_LIST_SUCCESS", response);
    });
  },
  getById: (context, params) => {
    return axios.get(`${DEVICE_API}/${params}`).then(response => {
      context.commit("GET_INFO_SUCCESS", response);
    });
  },
  add: (context, params) => {
    return axios.post(DEVICE_API, params).then(response => {
      context.commit("ADD_NEW_SUCCESS", response);
    });
  },
  update: (context, params) => {
    return axios.put(`${DEVICE_API}/${params.id}`, params).then(response => {
    context.commit("UPDATE_INFO_SUCCESS", response);
    });
  },
  deleteById: (context, params) => {
    return axios.delete(`${DEVICE_API}/${params}`).then(() => {
