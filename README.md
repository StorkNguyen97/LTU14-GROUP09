...
* `docker run --network emoji-net -it -p 8080:8080 -v $PWD:/root/project -w /root/project myapp-run sh -c .build-ubuntu/release/EmojiJournalServer`

> **Note:** Network and CouchDB commands added by Audrey Tam, so this works for the Docker tutorial. The CouchDB container *must* be named "couchdb", because this is what EmojiJournalServer expects. It's hard-coded in **Application.swift** as the host name in the `connectionProperties` property of the `App` class.
